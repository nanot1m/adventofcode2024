 ...
 */ function draw(input, ctx, part2) {
    ...
    const { result, visited, end, start } = (0, _16Js.findPathWithPredecessors)(map);
    const drawMap = ()=>{
        unsubscribe();
...
},{"../../../js/modules/iterator-extensions.js":"8eMUF","../../../js/solutions/16.js":"jDBGZ","../../../js/modules/index.js":"Zicik","../common.js":"8wzUn"}],"jDBGZ":[function(require,module,exports,__globalThis) {
// @ts-check
...
function part1(input) {
    ...
    const result = findOptimalRoute(input, start).find((v)=>(0, _indexJs.V).eq(v.value.pos, end));
    return result.distance;
}
/**
 * @param {string[][]} input
 * @param {V.Vec2} start
 */ function findOptimalRoute(input, start) {
    return (0, _graphJs.dijkstra)((x)=>[
            {
        ...
        ].filter((v)=>(0, _indexJs.Array2d).get(input, v.pos) !== "#"), (x)=>x.cost, [
        {
            pos: start,
            cost: 0,
            dir: 1
        }
    ], (x)=>`${x.pos[0]},${x.pos[1]},${x.dir}`);
}
function part2(input) {
    const { result, visited, start, end } = findPathWithPredecessors(input);
    // console.log(result.distance)
    // const debugMap = Array2d.clone(input)
    // for (const i of visited) {
    // 	const v = i.value.pos
    // 	Array2d.set(debugMap, v, asciiRed("O"))
    // 	if (V.eq(v, start)) Array2d.set(debugMap, v, asciiGreen("S"))
    // 	if (V.eq(v, end)) Array2d.set(debugMap, v, asciiBlue("E"))
    // }
    // console.log(Array2d.toString(debugMap))
    return visited.values().distinct((x)=>(0, _indexJs.V).toString(x.value.pos)).count();
}
const asciiBlue = (/** @type {string} */ str)=>`\x1b[34m${str}\x1b[0m`;
const asciiGreen = (/** @type {string} */ str)=>`\x1b[32m${str}\x1b[0m`;
const asciiRed = (/** @type {string} */ str)=>`\x1b[31m${str}\x1b[0m`;
function findPathWithPredecessors(input) {
    const start = (0, _indexJs.Array2d).traverse(input).find((v)=>v.value === "S").pos;
    const end = (0, _indexJs.Array2d).traverse(input).find((v)=>v.value === "E").pos;
    const result = findOptimalRoute(input, start).find((v)=>(0, _indexJs.V).eq(v.value.pos, end));
    /** @type {Set<import("../modules/graph.js").PathItem<{pos: V.Vec2, cost: number, dir: number}>>} */ const visited = new Set();
    /**
	 * @param {import("../modules/graph.js").PathItem<{pos: V.Vec2, cost: number, dir: number}>} item
	 */ function backtrack(item) {
        visited.add(item);
        for (const p of item.predecessors)backtrack(p);
    }
    backtrack(result);
    return {
        result,
        visited: Array.from(visited).sort((a, b)=>a.distance - b.distance),
        start,
        end
    };
}

},{"../modules/graph.js":"hSzmA","../modules/index.js":"Zicik","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9LZNA","jbdHw"], "jbdHw", "parcelRequire94c2")
